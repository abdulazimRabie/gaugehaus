...
const resetPasswordTemplate = fs.readFileSync(
  path.join(__dirname, "../views/emails/reset-password.html"),
  "utf8",
);
...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  if (req.body.images) newUser.images = req.body.images;
  ...
  try {
    ...
    return next(
      new AppError(
        "Something went wroing while creating profile, try again",
        400,
      ),
    );
  }
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !bcrypt.compare(password, user.password))
    return next(new AppError("Email / password is wrong", 401));
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError("Not authorized to access this route. Log in.", 401),
    );
  }
...
exports.verifyOTP = catchAsync(async (req, res, next) => {
  ...
  if (!email || !resetOTP) {
    return next(
      new AppError("Please provide email and OTP to reset password.", 400),
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password || !confirmedPassword) {
    return next(
      new AppError(
        "Please provide email and new password then confirm password.",
        400,
      ),
    );
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!oldPassword || !password || !confirmedPassword) {
    return next(
      new AppError(
        "Please provide old password and new password then confirm new one.",
        400,
      ),
    );
  }
